...
export class CodexRunner implements AgentRunner {
  ...
  async isAvailable(): Promise<boolean> {
    try {
      await execFileAsync(this.config.codexBin, ["--version"], {
        timeout: 5_000,
        env: this.childEnvironment(this.config.codexHome, false),
      });
      return true;
    } catch {
      return false;
  ...
  async run(request: RunnerRequest): Promise<RunnerResult> {
    ...
    const sessionHome = await prepareCodexSessionHome(this.config, request, true);
    const modelCredential = modelCredentialForRun(this.config, request);
    const args = buildCodexArgs(request, this.config.codexSandboxMode);
    const child = spawn(this.config.codexBin, args, {
      cwd: request.workspacePath,
      env: this.childEnvironment(sessionHome, true, modelCredential),
