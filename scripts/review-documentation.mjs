...
const requirements = {
  README: [
    ...
    /demo:verify-video[\s\S]{0,900}external-demo-video-review-attestation[\s\S]{0,420}Ed25519/i,
    /manual-secret-review[\s\S]{0,180}does not turn[\s\S]{0,160}verified evidence/i,
    /audit:source-delivery[\s\S]{0,420}does not prove reviewer access/i,
  ...
  limitations: [
    ...
    /recorded Ark clean-clone report verified 10\/10 product scenarios/i,
  ...
  deployment: [
    ...
    /audit:source-delivery[\s\S]{0,520}(?:read permission|评委可访问)/i,
    /Ed25519[\s\S]{0,220}attestation/i,
    /manual flag[\s\S]{0,80}不会自证通过/i,
